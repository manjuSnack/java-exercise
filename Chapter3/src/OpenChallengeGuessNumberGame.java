 ...
 * @author manjuSnack
 * 
 * OpenChallenge 카드 번호 맞추기 게임(up & down 게임)
 */
public class OpenChallengeGuessNumberGame {

	public static void main(String[] args) {
		int low, high; // 카드에 적힌 수의 범위
		int card; // 카드에 적힌 번호 값(숨기는 값, 정답)
		Random random = new Random(); // Random(난수) 발생기 생성
		...
		while(true) {
			...
			low = 0;
			high = 99;
			card = random.nextInt(100); // 0과 99 사이의 정수 난수(숨기는 값) 생성
			...
			while(true) {
				...
				catch (InputMismatchException e) { // 키 입력을 정수로 변환하지 못하는 예외처리
				...
				if(n>high || n<low) // high와 low의 범위를 벗어난 경우
					...
					if(n==card) {
						...
						// 입력된 값이 정답보다 높은 범위의 수인 경우
						System.out.println("더 낮게");
						high = n;
					} else {
						// 입력된 값이 정답보다 낮은 범위의 수인 경우
			...
			if(scanner.next().equals("no")) // yes인 경우 while 문으로 게임을 다시 시작(반복)할 수 있다.
				break; // while문 종료
		} // end of while
