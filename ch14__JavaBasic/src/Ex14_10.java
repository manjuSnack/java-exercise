// #14.52 스트림의 분할 - partitioningBy( ) 예제
...
public class Ex14_10 {
    public static void main(String[] args) {
        ...
        out.printf("1. 단순분할(성별로 분할)%n");
        Map<Boolean, List<Student2>> stuBySex = Stream.of(stuArr)
                .collect(partitioningBy(Student2::isMale)); // 학생들을 성별로 분할

        List<Student2> maleStudent2 = stuBySex.get(true); // Map에서 남학생 목록을 얻어온다.
        List<Student2> femaleStudent2 = stuBySex.get(false); // Map에서 여학생 목록을 얻어온다.
        ...
        out.printf("%n2. 단순분할 + 통계(성별 학생수)%n");
        Map<Boolean, Long> stuNumBySex = Stream.of(stuArr)
                .collect(partitioningBy(Student2::isMale, counting())); // 성별에 따른 학생 수

        out.println("남학생 수 : " + stuNumBySex.get(true));
        out.println("여학생 수 : " + stuNumBySex.get(false));

        out.printf("%n3. 단순분할 + 통계(성별 1등)%n");
        Map<Boolean, Optional<Student2>> topScoreBySex = Stream.of(stuArr)
                .collect(partitioningBy(Student2::isMale, maxBy(comparingInt(Student2::getScore))));

        out.println("남학생 1등 : " + topScoreBySex.get(true));
        out.println("여학생 1등 : " + topScoreBySex.get(false));

        Map<Boolean, Student2> topScoreBySex2 = Stream.of(stuArr)
                .collect(partitioningBy(Student2::isMale,
                        collectingAndThen(maxBy(comparingInt(Student2::getScore)), Optional::get)));

        out.println("남학생 1등 : " + topScoreBySex2.get(true));
        out.println("여학생 1등 : " + topScoreBySex2.get(false));

        out.printf("%n4. 다중분할(성별 불합격자, 100점 이하)%n");
